...
const evidence = [
  ...
  {
    id: "mace", baseline: "PREVENT base · ASCVD · 10 or 30 years",
    baselineSources: ["prevent"],
    sources: { sglt2i: "smartc-glp1", nsmra: "neuen", glp1: "lee2025" },
    limitation: "* Baseline risk is PREVENT ASCVD (atherosclerotic cardiovascular disease), covering MI, fatal coronary disease and stroke. Treatment effects use trial MACE (major adverse cardiovascular events) risk reduction as a proxy for ASCVD risk reduction. MACE includes nonfatal MI, nonfatal stroke and cardiovascular death, including other CV causes. These endpoints differ, so the displayed ASCVD risk reduction and treated risk are approximations, not direct ASCVD treatment-effect estimates.",
  },
  {
    ...
    <details className="group min-w-0 rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="space-y-4 border-t border-slate-100 p-3 text-[11px] leading-relaxed text-muted">
        ...
        <div className="space-y-1">
          ...
          <p>Cardiovascular-death baseline uses the coronary and noncoronary fatal-event equations from <SourceLink id="score" />, with eGFR and UACR adjustments from <SourceLink id="ckd-patch" />. It remains a 10-year estimate when the HF/ASCVD horizon is changed. The calibration is fixed to low-risk Europe and is independent of the KFRE region setting; it is not a Canadian calibration. This implementation requires eGFR and UACR and uses the reference tool’s selectable input limits: age 40–65, SBP 90–180 mm Hg, eGFR 15–120, UACR 5–1500 mg/g and cholesterol 3.879–7.241 mmol/L. These are implementation limits, not the full age range of the CKD-PC cohorts.</p>
          ...
          <p>PREVENT is for people without known CVD, ages 30–79 for 10-year risk and 30–59 for 30-year risk. The base model is used here. KFRE uses age, sex, eGFR and UACR and is shown only for CKD G3–G5 (eGFR below 60). Age entry is 18–100 years; 100 is an application limit, not a published KFRE validation boundary. Grey input ranges cover the widest supported range across models; each result applies its own narrower limits. The region selection changes KFRE calibration only. ACR conversion: mg/mmol × 8.84 = mg/g. Cholesterol conversion: mmol/L × 38.67 = mg/dL.</p>
        </div>
